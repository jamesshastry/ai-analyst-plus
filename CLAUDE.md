...
| Codex Review (legacy Claude) | `.claude/skills/codex-review/SKILL.md` | Invoked in Claude Code as `/codex-review` or "validate with codex" — a second model (Codex) independently re-derives the current analysis from the same data (blind to Claude's numbers) and reports AGREE/DISAGREE/PARTIAL per finding. This is the legacy Claude plugin workflow. Codex users should use `$independent-review` for provider-neutral validation or `$claude-review` to have Claude validate a Codex result; Codex-native audit trails are `.knowledge/independent-review/` and `.knowledge/claude-review/`. |
